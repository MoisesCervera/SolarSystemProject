class GameContext:
    """
    Singleton que mantiene el estado global de la sesión de juego.
    Actúa como una 'pizarra' compartida entre los diferentes estados y entidades.
    """
    _instance = None

    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(GameContext, cls).__new__(cls)
            cls._instance._initialized = False
        return cls._instance

    def __init__(self):
        if self._initialized:
            return

        self._initialized = True
        self.reset()

    # Class-level attribute for selected ship (accessible without instance)
    selected_ship = 'shipM'  # Default ship
    orbital_only = False  # Flag for orbital-only mode

    def reset(self):
        """Reinicia los datos de la sesión a sus valores por defecto."""
        # ID del personaje/nave seleccionado (0, 1, 2)
        self.selected_character_id = 0

        # Selected ship model ID
        GameContext.selected_ship = 'shipM'

        # Planeta objetivo actual (para visitar/escanear)
        self.target_planet = None

        # Reset orbital only flag
        GameContext.orbital_only = False

        # Reset mission manager if it exists
        try:
            from src.core.mission_manager import MissionManager
            mission_mgr = MissionManager()
            mission_mgr.reset()
            print("[Session] Mission manager reset")
        except ImportError:
            pass

    def set_character(self, char_id):
        self.selected_character_id = char_id
        print(f"[Session] Personaje seleccionado: {char_id}")

    @classmethod
    def set_ship(cls, ship_id):
        cls.selected_ship = ship_id
        print(f"[Session] Ship selected: {ship_id}")
